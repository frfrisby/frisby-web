 ...
 * responsible for body assembly and dispatching PUT-specific requests via the
 * shared {@link HttpEngine}.
 */
final class PutRequest implements PutSpec {
    ...
    @Override
    public PutSpec path(String path) {
        state.path(path);
        return this;
    }

    @Override
    public PutSpec path(String path, String parameterId, String parameterValue) {
        state.path(path, parameterId, parameterValue);
        return this;
    }

    @Override
    public PutSpec path(String path, PathParameter... parameters) {
        state.path(path, parameters);
        return this;
    }

    @Override
    public PutSpec parameter(String name, String value) {
        state.parameter(name, value);
        return this;
    }

    @Override
    public PutSpec parameter(String name, String... values) {
        state.parameter(name, values);
        return this;
    }

    @Override
    public PutSpec header(String name, String value) {
        state.header(name, value);
        return this;
    }

    @Override
    public PutSpec header(String name, String... values) {
        state.header(name, values);
        return this;
    }

    @Override
    public PutSpec cookie(HttpCookie cookie) {
        state.cookie(cookie);
        return this;
    }

    @Override
    public PutSpec security(SecurityProvider provider) {
        state.security(provider);
        return this;
    }

    @Override
    public PutSpec compress() {
        return compress(GZIP_COMPRESSOR);
    }

    @Override
    public PutSpec compress(ContentCompressor compressor) {
    ...
    public <T> HttpResponse<T> send(Class<T> responseType) {
        ...
        return engine.send(
                outbound,
                JsonBodyHandler.of(engine.configuration().serializer(), responseType, PUT, uri,
    ...
    public HttpResponse<Void> send() {
        ...
        return engine.send(outbound, RequestState.voidBodyHandler(PUT, uri));
    }
    ...
    public CompletableFuture<HttpResponse<Void>> sendAsync() {
        ...
        return engine.sendAsync(outbound, RequestState.voidBodyHandler(PUT, uri));
    }
