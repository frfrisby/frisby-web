...
/**
 * A builder for creating a {@link ServerLoggingConfiguration} instance.
 * <p>
 * Obtain a builder via {@link ServerLoggingConfiguration#builder()}.
 *
 * <pre>{@code
 * ServerLoggingConfiguration logging = ServerLoggingConfiguration.builder()
 *         .maxBodySize(4096)
 *         .redactHeaders("X-Amzn-Oidc-Data", "X-Api-Key")
 *         .redactFields("password", "token")
 *         .build();
 * }</pre>
 *
 * @see ServerLoggingConfiguration
 */
public interface ServerLoggingConfigurationBuilder {
    /**
     * Adds HTTP header names to the masking set used in failure log entries.
     * <p>
     ...
     * }</pre>
     * <p>
     * Optional.  By default, only the three built-in headers are masked.
     ...
     */
    ServerLoggingConfigurationBuilder redactHeaders(String... headers);

    /**
     * Specifies body field names whose string values will be replaced with
     * {@code [redacted]} in failure log entries.
     * <p>
     * Redaction applies to request bodies across two content types:
     * <ul>
     ...
     * @throws software.frisby.core.validation.BlankValueException      if any element is blank.
     */
    ServerLoggingConfigurationBuilder redactFields(String... fields);

    /**
     * Sets the maximum number of bytes buffered from an incoming request body and
     * included in failure log entries.
     * <p>
     * Bodies larger than this limit are truncated and marked in the log.
     * Pass {@code 0} to suppress request body logging entirely.
     * <p>
     * Optional.  Defaults to {@code 8192} (8 KB).
     *
     * @param maxBodySize The maximum body size in bytes; must be between {@code 0}
     ...
     *                                                                           the allowed range.
     */
    ServerLoggingConfigurationBuilder maxBodySize(int maxBodySize);

    /**
     * Returns a new {@link ServerLoggingConfiguration} instance based on the options
     * configured on this builder.
     *
     * @return A new {@link ServerLoggingConfiguration}; never {@code null}.
     */
    ServerLoggingConfiguration build();
}
